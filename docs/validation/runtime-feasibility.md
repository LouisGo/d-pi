...
验证日期：2026-09-24。本阶段已完成；范围来自用户当时指令。先确认 Runtime 能力及接入成本，再投入 GUI 产品化。旧阶段 1 的全部验收、视觉打磨和前端状态管理不是本轮门槛。
...
- [旧阶段 1 记录](../archive/stage1-evidence.md)保留可见 GUI、真实模型与本地 `.app` 开发闭环的历史证据；对应原型代码已移除，可从 Git 提交 `6fab3ef` 查阅。本阶段另外完成了上文的最小随包验证，不代表重新完成产品 GUI 验收。
...
最小随包探针已验证从应用资源目录运行 OMP，配置/会话位于可写目录。早期依赖外部 OMP 的应用实现及构建配置已移除，仓库目前不含产品应用。后续产品构建应遵循受管理资源路径和原生配置策略，发行与首次启动仍须验收。本轮运行环境是开发机器，不宣称没有其他系统依赖。
...
随包最小启动已完成独立验证；自动记忆质量、长输出性能和完整控制面板属于后续有具体需求时再验证的范围。本轮不扩展这些工作。
